...
/**
 * Noto Sans SC 的字体度量（单位 em，实测自 public/fonts/noto-sans-sc-*.otf）。
 * react-pdf 把基线固定放在「行盒顶部 + ascent」处，而 ascent(1.16em) 远大于汉字字面
 * 顶部(0.86em)，所以默认行盒里的汉字会明显偏下；如果外层再加背景色块，就会看起来
 * 「文字贴着底边、上下不居中」。
 */
const FONT_ASCENT_EM = 1.16
const CJK_GLYPH_TOP_EM = 0.86
const CJK_GLYPH_BOTTOM_EM = 0.091
/** 行盒顶部到汉字字面顶部的空隙 */
const CJK_TOP_GAP_EM = FONT_ASCENT_EM - CJK_GLYPH_TOP_EM
/**
 * 让汉字在自身行盒内上下居中所需的 lineHeight 倍数：
 * 上空隙(0.30em) + 字面高度(0.951em) + 等量下空隙(0.30em) ≈ 1.551em
 */
const CJK_BALANCED_LINE_HEIGHT =
  CJK_TOP_GAP_EM * 2 + CJK_GLYPH_TOP_EM + CJK_GLYPH_BOTTOM_EM
  ...
  return (
    <>
      {lines.map((line, lineIndex) => (
          <View key={`${keyPrefix}-ln-${lineIndex}`} style={styles.wrappedTextRow}>
            {lineIndex === 0 ? <Text style={styles.label}>{label}</Text> : null}
           {renderInlineMarkdownTokens(styles, line, `${keyPrefix}-${lineIndex}`)}
        </View>
      ))}
    </>
  )
...
function buildFileName(modules: ResumeModule[], resumeId?: number, options?: ResumePdfOptions) {
  ...
  const baseName = preferredTitle || segments.join('-') || (resumeId ? `resume-${resumeId}` : 'Quick简历编辑器')
...
function ResumePdfDocument({
  ...
  /**
   * filled（校园技术蓝 / slate / executive）标题：背景色块交给外层 View，
   * 内层 Text 用「平衡行高」把汉字压回行盒正中，避免文字贴底。
   */
  const usesFilledHeadingSurface = (resolvedThemeConfig.templateId === 'slate' || isCampusBlue)
    && resolvedThemeConfig.headingStyle === 'auto'
  const filledHeadingBackgroundColor = usesFilledHeadingSurface
    ? theme.chipBackgroundColor
    : theme.sectionTitleColor
  const filledHeadingTextColor = usesFilledHeadingSurface
    ? theme.sectionTitleColor
    : '#f8fafc'
  // 平衡行高本身已带来 0.30em 的上下留白，这里只补差额，保证色块高度和原设计一致
  const filledHeadingExtraPadding = Math.max(
    0,
    (isCampusBlue ? 3 : 4) - CJK_TOP_GAP_EM * theme.sectionTitleSize
  )
  const filledHeadingSurfaceStyle = {
    backgroundColor: filledHeadingBackgroundColor,
    paddingHorizontal: isCampusBlue ? 10 : 8,
    paddingTop: filledHeadingExtraPadding,
    paddingBottom: filledHeadingExtraPadding,
  ...
  const filledHeadingTextStyle = {
    fontSize: theme.sectionTitleSize,
    fontWeight: 700 as const,
    color: filledHeadingTextColor,
    lineHeight: CJK_BALANCED_LINE_HEIGHT,
  }
  ...
  const renderSectionTitle = (title: string) => (
    sectionHeadingVariant === 'filled' ? (
      <View style={filledHeadingSurfaceStyle}>
        <Text style={filledHeadingTextStyle}>{title}</Text>
      </View>
    ) : (
  ...
  const renderHeaderBlock = (styleOverrides: Array<{ marginBottom?: number }> = []) => (
    basicInfo ? (
      <View style={[...headerContainerStyle, ...styleOverrides]}>
        ...
        {/* 联系方式：两列网格，标签固定宽右对齐，值统一左对齐起点 */}
        <View style={{ flexDirection: 'row', flexWrap: 'wrap', marginTop: 2 }}>
          {[
            { label: '邮箱', value: basicInfo.email },
            { label: '手机号', value: basicInfo.phone },
            { label: '微信', value: basicInfo.wechat },
            { label: '意向城市', value: basicInfo.targetCity },
            { label: '期望薪资', value: basicInfo.salaryRange },
            { label: '到岗时间', value: basicInfo.expectedEntryDate },
            ...
            .map((field) => (
              <View key={field.label} style={{ width: '50%', flexDirection: 'row', marginBottom: 2 }}>
                <Text style={[...headerLabelStyle, { width: 72, textAlign: 'right' }]}>{field.label}：</Text>
                <Text style={{ flexGrow: 1, flexBasis: 0, flexShrink: 1 }}>{field.value}</Text>
              </View>
            ))}
          {basicInfo.github ? (
            <View style={{ width: '50%', flexDirection: 'row', marginBottom: 2 }}>
              <Text style={[...headerLabelStyle, { width: 72, textAlign: 'right' }]}>GitHub：</Text>
              <Link src={normalizeExternalUrl(basicInfo.github)} style={headerLinkStyle}>{basicInfo.github}</Link>
            </View>
          ) : null}
          {basicInfo.blog ? (
            <View style={{ width: '50%', flexDirection: 'row', marginBottom: 2 }}>
              <Text style={[...headerLabelStyle, { width: 72, textAlign: 'right' }]}>博客：</Text>
              <Link src={normalizeExternalUrl(basicInfo.blog)} style={headerLinkStyle}>{basicInfo.blog}</Link>
            </View>
          ) : null}
        </View>
        {basicInfo.summary ? (
          <Text style={[styles.paragraph, { marginTop: 4 }, ...(isExecutive ? [{ color: '#e2e8f0' }] : [])]}>
    ...
    basicInfo ? (
      <View style={[styles.header, { marginBottom: theme.headerBottom }]}>
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12 }}>
          ...
          <View
            style={{
              minWidth: hasPhoto ? 78 : 92,
              maxWidth: hasPhoto ? 116 : 140,
              flexShrink: 0,
              justifyContent: 'center',
              paddingRight: 10,
              marginRight: 2,
              borderRightWidth: 1,
              borderRightColor: theme.sectionTitleBorderColor,
            }}
          >
            <Text
              style={{
                ...
                fontWeight: 700,
                color: theme.bodyColor,
                // 用平衡行高，避免 21pt 姓名的基线冲出行盒、和右侧联系方式错位
                lineHeight: CJK_BALANCED_LINE_HEIGHT,
                letterSpacing: 0.6,
              }}
            >
