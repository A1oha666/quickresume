...
const PREVIEW_PANEL_COLLAPSED_STORAGE_KEY = 'quickresume.preview-panel-collapsed'
const COMPACT_PREVIEW_MEDIA_QUERY = '(max-width: 1279px)'
const DESKTOP_MODULE_SIDEBAR_MEDIA_QUERY = '(min-width: 768px)'
const RESUME_PDF_PREVIEW_CONFIG_STORAGE_KEY_PREFIX = 'quickresume.pdf-preview-config'
...
export default function EditorPage() {
  ...
  useEffect(() => {
    ...
    const currentQueryView: EditorView = currentQueryViewParam === 'chrome-preview' || currentQueryViewParam === 'template-selection'
      ? 'template-selection'
      : 'module'
  ...
  const mobileWorkspaceLabel = editorView === 'template-selection'
    ? '预览与导出'
    : activeModuleType
  ...
  const handleExportWord = useCallback(async () => {
    if (modules.length === 0) {
      setExportError('请先完善简历内容后再导出 Word')
      return
    }
    ...
    try {
      ...
      const latestModules = useResumeStore.getState().modules
      if (latestModules.length === 0) {
        throw new Error('请先完善简历内容后再导出 Word')
      }
      await downloadResumeDocx(latestModules, resumeId, { documentTitle: resumeTitle })
    } catch (err: unknown) {
      const message = err instanceof Error ? err.message : '导出 Word 失败，请稍后重试'
      setExportError(message)
    } finally {
  ...
  const handleExportMd = useCallback(async () => {
    ...
    try {
      ...
      downloadResumeMd(latestModules, resumeId, { documentTitle: resumeTitle })
    } catch (err: unknown) {
  ...
  const handleSyncToFile = useCallback(async () => {
    if (modules.length === 0) {
      setSyncNotice('请先完善简历内容后再同步')
      return
    }
    setSyncingToFile(true)
    setSyncNotice('')
    try {
      await flushResumeAutoSaves(resumeId)
      const ok = await syncResumeToFileNow()
      setSyncNotice(ok
        ? '已同步到文件：简历-最新.md（工作区根目录）'
        : '同步失败：请确认同步服务（3456 端口）已启动')
    } catch {
      setSyncNotice('同步失败：请确认同步服务（3456 端口）已启动')
    } finally {
      setSyncingToFile(false)
    }
  }, [modules.length, resumeId])

  const handleRestoreFromFile = useCallback(async () => {
    const confirmed = window.confirm('将用服务端文件备份覆盖当前浏览器里的简历数据（本地未保存的改动会丢失），确定继续吗？')
    if (!confirmed) return
    try {
      const ok = await restoreResumeFromFile()
      if (!ok) {
        setSyncNotice('没有找到文件备份，请先在编辑页点「同步到文件」')
        return
      }
      window.location.reload()
    } catch {
      setSyncNotice('恢复失败：请确认同步服务（3456 端口）已启动')
    }
  }, [])
            ...
            <div className={moduleContainerClassName}>
              ...
              <div className="mb-4 flex flex-wrap items-center justify-end gap-2">
                <button
                  ...
                  title="用服务端文件备份覆盖当前浏览器数据"
                >
                  恢复备份
                </button>
                <button
                  type="button"
                  onClick={() => void handleSyncToFile()}
                  disabled={syncingToFile}
                  className="rounded-lg border border-primary-300 bg-primary-50 px-3 py-1.5 text-xs font-medium text-primary-700 hover:bg-primary-100 disabled:opacity-60"
                  title="把当前简历写入 简历-最新.md"
                >
                  {syncingToFile ? '同步中...' : '⇅ 同步到文件'}
                </button>
                ...
                <div className="space-y-4">
                  {activeModules.map((mod, index) => (
                    <div key={mod.id} className="editor-form-container rounded-xl border border-gray-200 bg-white p-4 sm:p-5">
                      ...
                      {activeModules.length === 1 && canDeleteActiveModule && (
